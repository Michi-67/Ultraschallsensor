...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  // Check if the previous read was a valid measurement (less than 0xFFFC means no error/out-of-range flag)
	  if (last_value < 0xFFFC)
	  {
	      // Check PD3 pin state to determine operating mode
	      if (HAL_GPIO_ReadPin(GPIOD, GPIO_PIN_3) == GPIO_PIN_SET)
	      {
	          // --- Temperature Mode ---
	          last_value = Sensor_ReadData(CMD_TEMP);
	          Write_7Seg(last_value, 2);
	      }
	      else
	      {
	          // --- Distance Mode ---
	          last_value = Sensor_ReadData(CMD_DISTANCE);
	          Write_7Seg(last_value, 4);
	      }
	  }
	  else
	  {
	      // --- Error / Out-of-Range Handling ---
	      // Previous read was an error value (0xFFFF, 0xFFFE, etc.)
	      // Subtract 55536 to shift the error flag down to a readable status code (e.g., 0xFFFF becomes 9999)
	      Write_7Seg((last_value - 55536), 4);

	      // rescan to see, if the error is fixed
	      last_value = Sensor_ReadData(CMD_TEMP);
	  }

	  // Global sampling delay between sensor cycles
	  HAL_Delay(100);
  /* USER CODE END 3 */
  }
}
...
static void MX_TIM2_Init(void)
{
  ...
  htim2.Instance = TIM2;
  htim2.Init.Prescaler = 4096-1;
  htim2.Init.CounterMode = TIM_COUNTERMODE_UP;
  htim2.Init.Period = 48828-1;
